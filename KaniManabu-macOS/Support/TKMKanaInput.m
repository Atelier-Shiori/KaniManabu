...
//
// Note: Source modified to add support for macOS
//
...
- (NSString *)checkString:(NSString *)currentstring withReplacementString:(NSString *)replacementstring withOldRange:(NSRange)range withCurrentRange:(NSRange)currentRange useCurrentRange:(bool)useCurrentRange {
  ...
  if (currentRange.location > 0 && replacementstring.length == 1 && useCurrentRange) {
    ...
    unichar lastChar = [currentstring characterAtIndex:currentRange.location - 1];
  ...
  for (int i = 3; i >= 0; --i) {
    ...
    NSRange replacementRange = NSMakeRange(range.location - i, i);
      NSString *text = [NSString
            stringWithFormat:@"%@%@", [currentstring substringWithRange:replacementRange], replacementstring];
